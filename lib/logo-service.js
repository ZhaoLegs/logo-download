...
const CRAWLEE_STORAGE_DIR = process.env.CRAWLEE_STORAGE_DIR || path.join(os.tmpdir(), 'logo-download-crawlee');
const SEARCH_CACHE_TTL = 5 * 60 * 1000;
const DEFAULT_ICONS_CACHE_TTL = 7 * 24 * 60 * 60 * 1000;
const FINAL_RESULT_LIMIT = 48;
...
function normalizeAppStoreIconUrl(url, size = 1024) {
    if (!url) return '';
    ...
    try {
        ...
        return data.results
            ...
            .map((appResult) => {
                ...
                return {
                    ...appResult,
                    artworkUrlMax: normalizeAppStoreIconUrl(
                        appResult.artworkUrl512 || appResult.artworkUrl100,
                        1024
                    ),
...
function sortAndDedupeResults(results, searchTerm) {
    ...
    return deduped.slice(0, FINAL_RESULT_LIMIT).map(({ finalScore, sourcePriority, originalIndex, ...result }) => result);
}
...
export async function getDefaultBackgroundIcons() {
    if (
...
export async function searchApps(rawQuery) {
...
export async function fetchIconAsset(targetUrl) {
    if (!targetUrl) {
        const error = new Error('Missing required query parameter: url');
    ...
    if (parsedUrl.protocol !== 'https:' || !isAllowedIconHost(parsedUrl.hostname)) {
        const error = new Error('Icon host is not allowed');
        error.statusCode = 400;
        throw error;
    }
    ...
    if (!response.ok) {
        const error = new Error('Unable to fetch icon');
